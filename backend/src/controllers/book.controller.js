...
const updateBook = async (req, res) => {
  ...
  if (id) {
    ...
    await bookModel.update({ name, author, description, genre }, {
      where: {
        id
      }
    })
...
const searchBook = async (req, res) => {
  ...
  try {
    if (!q) {
      return res.json({ success: false, message: "No query was found in the string." });
    }
...
const getBooks = async (req, res) => {
  ...
  try {
    ...
    res.json({ success: false, message: "Error while fetching books." })
  }
}
